...
const IssueLogs: React.FC<IssueLogsProps> = ({ records, onDelete, onUpdateStatus, onEdit }) => {
  ...
  const [activeSignature, setActiveSignature] = useState<SignatureData | null>(null);
  const [showSignaturePad, setShowSignaturePad] = useState(false);

  useEffect(() => {
    if (selectedRecord) {
      forensicService.getSignature(selectedRecord.id).then(setActiveSignature);
    } else {
  ...
  const handleSaveSignature = async (signatureBase64: string) => {
    ...
    const signature: SignatureData = {
      ...
      auditorName: selectedRecord.auditorName,
      timestamp: new Date().toISOString(),
      signatureBase64,
      pdfHash: Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15)
    };

    await forensicService.saveSignature(signature);
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] w-full max-w-5xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col ring-1 ring-white/20">
            ...
            <div className="flex-1 overflow-y-auto p-8 lg:p-12">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div className="flex flex-col h-full">
                  ...
                  <div className="flex flex-col gap-4 pt-8 mt-auto">
                    {activeSignature ? (
                      <div className="bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-900/30 p-4 rounded-2xl flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 bg-emerald-500 rounded-full flex items-center justify-center text-white text-xs">✓</div>
                          <span className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">Digitally Signed</span>
                        </div>
                        <PDFDownloadLink
                          document={<AuditReportPDF record={selectedRecord} signature={activeSignature} />}
                          fileName={`Audit_Report_${selectedRecord.id}.pdf`}
                      ...
                      <button
                        ...
                        className="w-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-black py-4 rounded-xl shadow-lg transition-all uppercase text-[10px] tracking-widest hover:bg-blue-600 dark:hover:bg-slate-200"
                      >
                        Authorize with Digital Signature
                      </button>
                    )}
              ...
              <div className="mt-12 pt-12 border-t border-slate-100 dark:border-slate-800">
                <AuditHistoryTimeline recordId={selectedRecord.id} />
              </div>
            </div>
          </div>
        </div>
